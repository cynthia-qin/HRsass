...
import { saveUserDetails, getPersonalDetail, updatePersonal } from '@/api/employees'
export default {
  data () {
    return {
  ...
  methods: {
    ...
    async getUserInfoById () {
      ...
      if (this.userInfo.staffPhoto && this.userInfo.staffPhoto.trim()) {
        // 将获取到的员工头像传给通过ref获取到组件实例上的fileList
    ...
    // 保存更新后的员工基本信息
    async saveUser () {
      // 在保存员工信息之前先要获取到上传图片的数组，
      const fileList = this.$refs.staffPhoto.fileList
      if (fileList.some(item => !item.upload)) {
        // 就表示图片还没有上传成功
        this.$message.warning('还有图片没有上传成功')
        return
      }
